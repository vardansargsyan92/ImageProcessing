...
public class Histogram_Matcher_HCV implements PlugInFilter {
    ...
    public void run(ImageProcessor ip) {
        ...
        int width = ip.getWidth(), height = ip.getHeight(), pixel, h, s, v;
        Color color;
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                color = new Color(ip.getPixel(col, row));
                int[] hsv = RGBtoHSV(color.getRed(), color.getGreen(), color.getBlue());
                badRed[hsv[0]]++;
                badGreen[hsv[1]]++;
                badBlue[hsv[1]]++;
            }
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                s = hsv[1];
                v = hsv[2];
                int[] colorInt = {search(goodRedCumulativ, 0, goodRedCumulativ.length - 1, badRedCumulativ[h]),
                        search(goodGreenCumulativ, 0, goodGreenCumulativ.length - 1, badGreenCumulativ[s]),
                        search(goodBlueCumulativ, 0, goodBlueCumulativ.length - 1, badBlueCumulativ[v])};
    ...
    public int[] RGBtoHSV(double r, double g, double b) {
        ...
        min = (int)Math.round(Math.min(Math.min(r, g), b));
        max = (int)Math.round(Math.max(Math.max(r, g), b));
        ...
        // H
        if (r == max)
            h = (int) (g - b) / delta; // between yellow & magenta
        else if (g == max)
            h = (int)(2 + (b - r) / delta); // between cyan & yellow
        else
            h = (int)(4 + (r - g) / delta); // between magenta & cyan
